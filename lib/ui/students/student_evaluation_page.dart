...
class _StudentEvaluationPageState extends State<StudentEvaluationPage> {
  ...
  // Cached streams to prevent rebuilds
  late Stream<List<StudentChecklist>> _allChecklistsStream;
  late Stream<ChecklistTemplate?> _templateStream;
  late Stream<StudentChecklist?> _currentChecklistStream;
  ...
  void initState() {
    ...
    // Initialize streams
    _allChecklistsStream = _checklistRepo.streamAllStudentChecklists(
      widget.student.id,
    );
    _updateCapStreams();

    _checklistRepo
        .ensureStudentChecklistInitialized(widget.student.id, _selectedCap)
        .catchError((e) {
  ...
  void _updateCapStreams() {
    _templateStream = _checklistRepo.streamTemplate(_selectedCap);
    _currentChecklistStream = _checklistRepo.streamStudentChecklist(
      widget.student.id,
      _selectedCap,
    );
  }

  Future<void> _updateItemProgress(
    CapLevel cap,
    String itemId,
    int score,
    bool completed,
  ...
  Future<void> _promoteToNextCap() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Deseja promover ${widget.student.name} para a touca ${next.name.toUpperCase()}?',
        ),
        actions: [
          TextButton(
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '${widget.student.name} foi promovido(a) para ${next.name.toUpperCase()}!',
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: AppBackground(
        child: StreamBuilder<List<StudentChecklist>>(
          stream: _allChecklistsStream,
                          ...
                          SingleChildScrollView(
                            ...
                            child: Row(
                              children: allChecklists.map((checklist) {
                                final isSelected =
                                    checklist.cap == _selectedCap;
                                return Padding(
                                  padding: const EdgeInsets.only(right: 8),
                                  child: InkWell(
                                    onTap: () {
                                      setState(() {
                                        _selectedCap = checklist.cap;
                                        _updateCapStreams();
                                      });
                                      _checklistRepo
                  ...
                  StreamBuilder<ChecklistTemplate?>(
                    ...
                    builder: (context, tplSnap) {
                      ...
                      return StreamBuilder<StudentChecklist?>(
                        stream: _currentChecklistStream,
                        builder: (context, progSnap) {
                          if (progSnap.connectionState ==
                          ...
                          final progressMap = {
                            for (var p in prog.items) p.itemId: p,
                          };
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: completed
            ...
            : const BorderSide(color: AppColors.lightDivider, width: 0.5),
      ),
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
